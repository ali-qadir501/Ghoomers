...
import { Star, MapPin, ShieldCheck, MessageCircle, ArrowUpRight, Award, TrendingUp, Clock, Calendar, Check } from 'lucide-react';
...
export const GuideCard: React.FC<GuideCardProps> = ({ guide, onSelect }) => {
  ...
  const [lastBooking, setLastBooking] = useState<{ date: string; trip: string } | null>(null);
  ...
  const getAvailabilityStatus = (availability: string[]) => {
    ...
    // Normalize availability array (handle 'Mon' vs 'Monday')
    const normalizedAvailability = availability.map(d => {
      const found = [
      ...
      ].find(ref => ref.full === d || ref.short === d);
      return found ? found.full : d;
    });

    if (normalizedAvailability.includes(today)) {
      return { 
    ...
    if (normalizedAvailability.includes(tomorrow)) {
      return { 
  ...
  const availability = getAvailabilityStatus(guide.availability);
    ...
    >
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          {guide.isVerified && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500 text-white text-[10px] font-black tracking-widest shadow-lg shadow-emerald-500/20">
              <ShieldCheck size={12} />
              VERIFIED
            </div>
          )}
          ...
          <div className={cn(
            "flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black tracking-widest shadow-lg backdrop-blur-md",
            availability.bg,
            availability.text
          )}>
            <Clock size={12} />
          ...
          {guide.popularity > 100 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-sky-500 text-white text-[10px] font-black tracking-widest shadow-lg shadow-sky-500/20">
              <TrendingUp size={12} />
              POPULAR
            </div>
          )}
          ...
          {lastBooking && (
            <motion.div 
              ...
              className="flex flex-col gap-1.5 px-3 py-2 rounded-xl bg-emerald-500 text-white shadow-lg shadow-emerald-500/20"
            >
              <div className="flex items-center gap-1.5 text-[10px] font-black tracking-widest">
                <Check size={12} />
                BOOKED!
              </div>
              <div className="text-[8px] font-bold opacity-90 leading-tight">
                {lastBooking.trip}<br/>
                {lastBooking.date}
              </div>
            </motion.div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="p-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-300 flex items-center justify-center"
              title="View Profile"
            >
