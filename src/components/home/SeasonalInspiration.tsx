...
export const SeasonalInspiration = () => {
  ...
  const [mode, setMode] = useState<'blossom' | 'autumn'>('blossom');
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      const prompt = mode === 'blossom' 
        ? "A photo of Hunza Valley in northern Pakistan during peak apricot blossom season in April. Thousands of white and pale pink apricot trees in terraced orchards cascade down mountain slopes. Baltit Fort perched on a rocky outcrop in the mid-ground. Rakaposhi peak (7,788m) snow-covered and dominant in the background. Early morning golden hour light."
        : "A photo of Ghizer Valley or Hunza in northern Pakistan during peak autumn season in late October. The poplars and apricot trees are brilliant gold, orange, and red. Contrast with the turquoise river and rugged brown mountains. Crisp clear sky. High resolution travel photography.";
          ...
          <div className="relative">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl md:text-5xl font-black text-slate-900 font-display mb-6 tracking-tight">
                Visualize the <span className={cn(
                  "text-transparent bg-clip-text bg-gradient-to-r transition-all duration-700",
                  mode === 'blossom' ? "from-emerald-600 to-teal-600" : "from-amber-600 to-orange-600"
                )}>
                  {mode === 'blossom' ? 'Blossom Season' : 'Autumn Colors'}
                </span>
              </h2>
              <p className="text-slate-500 text-lg font-medium leading-relaxed mb-8">
                {mode === 'blossom' 
                  ? "Experience the magic of Hunza Valley in April. Our AI can visualize the peak apricot blossom season, showing you the breathtaking white and pink terraces."
                  : "Witness the fiery transformation of the Karakoram.Late October brings gold and crimson leaves against deep turquoise rivers and snow-capped peaks."
                }
              </p>
              
              <div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl w-fit mb-8">
                <button
                  onClick={() => { setMode('blossom'); setGeneratedImage(null); }}
                  className={cn(
                    "px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
                    mode === 'blossom' ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}
                >
                  Spring Blossom
                </button>
                <button
                  onClick={() => { setMode('autumn'); setGeneratedImage(null); }}
                  className={cn(
                    "px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
                    mode === 'autumn' ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  )}
                >
                  Autumn Gold
                </button>
