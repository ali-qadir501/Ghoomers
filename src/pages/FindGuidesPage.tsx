...
import { Search, Filter, MapPin, Star, ShieldCheck, X, ChevronDown, Map as MapIcon, SlidersHorizontal, Calendar } from 'lucide-react';
...
export const FindGuidesPage: React.FC<{ onSelectGuide: (guide: Guide) => void }> = ({ onSelectGuide }) => {
  ...
  const locations = useMemo(() => ['All', ...Array.from(new Set((guides || []).map(g => g.location)))], [guides]);
  const activities = useMemo(() => ['All', ...Array.from(new Set((guides || []).flatMap(g => g.specialties || [])))], [guides]);
  ...
  const filteredGuides = useMemo(() => {
    ...
    let results = (guides || []).filter(guide => {
      ...
      const matchesRating = (guide.rating || 0) >= (advancedFilters.rating || 0);
      const matchesPrice = (guide.pricePerDay || 0) >= (advancedFilters.priceRange?.[0] || 0) && (guide.pricePerDay || 0) <= (advancedFilters.priceRange?.[1] || 1000);
      const matchesLanguages = (advancedFilters.languages?.length || 0) === 0 || advancedFilters.languages.some(l => (guide.languages || []).includes(l));
      const matchesCerts = (advancedFilters.certifications?.length || 0) === 0 || (guide.certifications && advancedFilters.certifications.some(c => guide.certifications!.includes(c)));
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 mb-12">
          <div className="max-w-2xl">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-lg font-medium max-w-xl">
              Real people from GB and Chitral showing you their world. No agencies, just raw, unpackaged experiences with verified local experts.
            </p>
          </div>
            ...
            <div className="w-full flex flex-col gap-6">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[32px] p-2 shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-stretch md:items-center gap-2">
                ...
                {/* Availability Filter */}
                <div className="flex-1 flex items-center px-6 py-3 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800 group relative">
                  <Calendar className="text-slate-400 mr-3 group-hover:text-sky-500 transition-colors" size={20} />
                  <div className="flex-1">
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none no-scrollbar">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mr-2 whitespace-nowrap">Quick Browse:</span>
                {days.map(day => (
                  <button
                    key={day}
                    ...
                    className={cn(
                      ...
                      selectedDay === day 
                        ...
                        : "bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-100 dark:border-slate-800 hover:border-sky-500 dark:hover:border-sky-500 hover:text-sky-500"
                    )}
                  >
